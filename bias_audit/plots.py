...
from .intersectional import SMALL_SAMPLE_NOTE, IntersectionalAudit, _truthy
...
def _figure(*args, **kwargs):
    """``plt.subplots`` that closes the figure if the caller raises.

    Matplotlib keeps every figure alive in a module-level registry until it is
    explicitly closed, so a plot helper that raises part-way through leaks one
    figure per call. Rendering a directory of results in a loop then grows
    without bound and eventually trips the "more than 20 figures" warning.
    """
...
def intersectional_heatmap(
    ...
    with _figure(figsize=(14, 5.5)) as (fig, ax):
...
def top_disadvantaged_bar(
    ...
    flag = f"{metric}_fair"
    # ``astype(bool)`` maps the strings "False"/"nan" that a CSV round-trip
    # leaves behind to True, colouring a failing subgroup green.
    fair_flags = _truthy(frame[flag]) if flag in frame.columns else pd.Series(True, index=frame.index)

    with _figure(figsize=(10, 0.5 * len(frame) + 2)) as (fig, ax):
        colours = ["#27ae60" if fair else "#c0392b" for fair in fair_flags]
...
def sample_size_scatter(
    ...
    with _figure(figsize=(9, 5.5)) as (fig, ax):
...
def single_vs_intersectional(
    ...
    if worst_intersectional.empty:
        raise ValueError("No evaluated groups to plot")
    if metric not in single.columns:
        single = single.iloc[0:0]

    with _figure(1, 2, figsize=(15, 0.45 * max(k, len(single)) + 3)) as (fig, (ax_left, ax_right)):
        if single.empty:
            ax_left.text(0.5, 0.5, "no single-attribute results", ha="center", va="center")
            ax_left.set_axis_off()
        else:
            labels = single["attribute"].astype(str) + ": " + single["group"].astype(str)
...
def fairness_accuracy_tradeoff(
    ...
    if tradeoff.empty:
        raise ValueError("tradeoff frame has no rows to plot")
